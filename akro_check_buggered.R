...
the.number.i.need.to.use.to.create.the.below.file<-ncol(centroids.complete)-1
...
newdata <- centroid.muns[with(centroid.muns,order(soil.id)),]
...
greek <- c("\U03B1", "\U03B2", "\U03B3", "\U03B4", "\U03B5", "\U03B6", "\U03B7", "\U03B8", "\U03B9", "\U03BA", "\U03BB", 
           "\U03BC", "\U03BD", "\U03BE", "\U03BF", "\U03C1", "\U03C2", "\U03C3", "\U03C4", "\U03C5", "\U03C6", "\U03C7", 
           "\U03C8", "\U03C9")
greek11<-make_letter_ids(11,lsource=greek)


greek.numeric<-1:11
neworder<-c(greek.numeric,mat.max)
alpha<-make_letter_ids(11, greek)
alphaorder<-c(alpha,mat.max)
greekorder<-c(greek11,mat.max)
...
#   
# ) #
##soil texture plot with USDA definitions:
geo <- TT.plot(class.sys="USDA.TT",main="Soil texture data -end points(red), centroids(blue)")
##to plot the triangle with no confusing words on it, use this version:
geo <- TT.plot(class.sys="none",main="Soil texture data -end points(red), centroids(blue)")
                                                                                                                    ...
                                                                                                                    ifelse(geo.cent$max=="r","u",
                                                                                                                           ifelse(geo.cent$max=="s","v",
                                                                                                                                  ifelse(geo.cent$max=="p","w",
                                                                                                                                         ifelse(geo.cent$max=="eta","alpha",#\U03B1",#"alpha",
                                                                                                                                                ifelse(geo.cent$max=="beta","beta",#\U03B2", #beta",
                                                                                                                                                       ifelse(geo.cent$max=="delta","gamma",#\U03B3", #gamma",
                                                                                                                                                              ifelse(geo.cent$max=="iota","delta",#\U03B4",#delta",
                                                                                                                                                                     ifelse(geo.cent$max=="epsilon","epsilon",#\U03B5",#epsilon",
                                                                                                                                                                            ifelse(geo.cent$max=="kappa","zeta",#\U03B6",#zeta",
                                                                                                                                                                                   ifelse(geo.cent$max=="theta","eta",#\U03B7",#eta",
                                                                                                                                                                                          ifelse(geo.cent$max=="alpha","theta",#\U03B8",#theta",
                                                                                                                                                                                                 ifelse(geo.cent$max=="lambda","iota",#\U03B9",#iota",
                                                                                                                                                                                                        ifelse(geo.cent$max=="zeta","kappa",#\U03BA",#kappa",                 
                                                                                                                                                                                                               ifelse(geo.cent$max=="gamma","lambda",0)))))))))))))))))))))))))))))#\U03BB"#lambda"
...
#geo.cent will throw an error.  Once location data is added, the file to be bound to geo.cent is a different length. .
...
for(i in c(mat.max,cent.max)){  
  pl<-ggplot(world_map, aes(x=x, y=y))+
    scale_x_continuous(limits=c(-175, -50))+
