...
subset5_10 <- subset(checkII, hzn_top.x >= 5 & hzn_top.x < 10)
subset10_20 <- subset(checkII, hzn_top.x >= 10 & hzn_top.x < 20)
subset20_40 <- subset(checkII, hzn_top.x >= 20 & hzn_top.x < 40)
subset40_60 <- subset(checkII, hzn_top.x >= 40 & hzn_top.x < 60)
subset60_100 <- subset(checkII, hzn_top.x >= 60 & hzn_top.x < 100)
subset100plus<-subset(checkII,hzn_top.x>=100 & hzn_top.x<10000)

##preparing for fuzzy k algorithm. No extraneous columns, keep an id column.
subset0_5<-subset0_5[,-5]
subset5_10<-subset5_10[,-5]
subset10_20<-subset10_20[,-5]
subset20_40<-subset20_40[,-5]
subset40_60<-subset40_60[,-5]
subset60_100<-subset60_100[,-5]
subset100plus<-subset100plus[,-5]

##outputting data into a csv
write.csv(subset0_5, "USII_0_5.csv")
write.csv(subset5_10, "USII_5_10.csv")
write.csv(subset10_20, "USII_10_20.csv")
write.csv(subset20_40, "USII_20_40.csv")
write.csv(subset40_60, "USII_40_60.csv")
write.csv(subset60_100, "USII_60_100.csv")
write.csv(subset100plus, "USII_100plus.csv")

###these can be fuzzy k'd to get optimal result.
...
prin<-a$scores
csprin<-cbind(class_input,prin0_5)
