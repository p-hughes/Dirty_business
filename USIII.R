...
contrast<-colorRampPalette(c("orange3","orangered","orangered4","orchid4","palegreen4","palevioletre"))
...
copdat43.1.2 <- read.delim("clipboard",sep="", header=T)
...
all.cent<-rbind(cent.muns.greek[1:23,],greek.cent)

ALLtex<-all.cent[,c("clay_tot_psa","sand_tot_psa","oc")]
...
ALLtex<-cbind(ALLtex,all.cent$soil.id)
ALLtex<-cbind(ALLtex,all.cent$natural_key)


with(all.cent,plot3d(L,A,B, main="centroid colours",col=max, size=3))
...
TT.text(
  ...
  font = 6,
  col = c("blue","red")[as.numeric(ALLtex[,5])]
...
pdf("USIII_max.pdf", height=5, width=10)
...
setwd("C:\\Users\\phug7649\\Desktop\\kmeans\\US_thickness_analysis_09072013\\a")
write.csv(data.complete,file="data and distances.csv")
write.csv(data.complete,file="centroid table.csv")